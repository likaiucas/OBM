...
class LoadAnnotations(object):
    ...
    def __init__(self,
        ...
        self.with_building_mask =with_building_mask
    ...
    def _load_building_masks(self, results):
        """Private function to load mask annotations.
        ...
        h, w = results['img_info']['height'], results['img_info']['width']
        gt_building_masks = results['ann_info']['building_masks']
        if self.poly2mask:
            gt_building_masks2 = BitmapMasks(
                [self._poly2mask(mask, h, w) for mask in gt_building_masks], h, w)
        else:
            gt_building_masks2 = PolygonMasks(
                [self.process_polygons(polygons) for polygons in gt_building_masks], h,
                w)
        results['gt_building_masks'] = gt_building_masks2
        results['mask_fields'].append('gt_building_masks')
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_building_mask:
            results = self._load_building_masks(results)
        if self.with_mask:
