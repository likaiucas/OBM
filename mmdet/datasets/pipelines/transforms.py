...
class Resize(object):
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            results[key] = bboxes
    def _resize_offset(self, results):
        gt_offsets = results.get('gt_offsets')
        if gt_offsets is not None:
            offsets = gt_offsets * results['scale_factor'][2:]
            results['gt_offsets'] = offsets
...
class RandomFlip(object):
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        ...
        if isinstance(direction, str):
            ...
            self.direction = np.random.choice(direction)
        else:
            raise ValueError(f"Invalid flipping direction '{direction}'")
        if flip_ratio is not None:
            assert flip_ratio >= 0 and flip_ratio <= 1
        assert direction in ['horizontal', 'vertical'] or direction == ['horizontal', 'vertical']
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('offset_fields', []):
                results[key] = self.offset_flip(results[key],
                                                  results['img_shape'],
                                                  results['flip_direction'])
        return results
...
class Expand(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('offset_field_fields', []):
            ...
            expand_gt_offset_field = np.full((int(h * ratio), int(w * ratio)),
                                    self.seg_ignore_label,
                                    dtype=gt_seg.dtype)
...
class RandomRotate(object):
    ...
    def offset_field_coordinate_transform(self, offset_field, transform_flag='xy2la'):
        ...
        if transform_flag == 'xy2la':
            ...
            offset_x = length * np.cos(angle)
            offset_y = length * np.sin(angle)
            offset_field_transformed[:, :, 0], offset_field_transformed[:, :, 1] = offset_x, offset_y
        else:
            raise NotImplementedError
    ...
    def __call__(self, results):
        ...
        if results['rotate']:
            ...
            for key in results.get('offset_field_fields', []):
                results[key] = self.offset_field_rotate(results[key],
                                                  results['img_shape'],
                                                  results['rotate_angle'])
            
            # rotate offsets
            for key in results.get('offset_fields', []):
                results[key] = self.offset_rotate(results[key],
                                                  results['img_shape'],
                                                  results['rotate_angle'])
        return results
...
class OffsetTransform(object):
    ...
    def __call__(self, results):
        ...
        # transform offsets
        for key in results.get('offset_fields', []):
            results[key] = self.transform_offset(results[key],
                                                 results['offset_coordinate'])
        return results
