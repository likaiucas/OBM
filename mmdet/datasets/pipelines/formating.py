...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        for key in ['proposals', 'gt_bboxes', 'gt_bboxes_ignore', 'gt_labels', 'gt_offsets', 'gt_building_heights', 'gt_angle', 'gt_rbboxes', 'gt_roof_bboxes', 'gt_footprint_bboxes', 'gt_only_footprint_flag', 'gt_roof_masks','gt_footprint_masks']:
            if key not in results:
                continue
            results[key] = DC(to_tensor(results[key]))
        if 'gt_masks' in results:
            results['gt_masks'] = DC(results['gt_masks'], cpu_only=True)
        if 'gt_building_masks' in results:
            results['gt_building_masks'] = DC(results['gt_building_masks'], cpu_only=True)
