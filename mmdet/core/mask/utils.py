...
def encode_mask_results(mask_results):
    ...
    if isinstance(cls_segms, torch.Tensor):
        # cls_segms = cls_segms.detach().cpu().numpy()
        if cls_segms.device!='cpu':
            cls_segms=cls_segms.to('cpu')
    ...
    for i in range(len(cls_segms)):
        for cls_segm in cls_segms[i]:
            ...
            encoded_mask_results[i].append(rle)  # encoded with RLE
