...
@BBOX_CODERS.register_module()
class DeltaXYOffsetCoder_Transformer(BaseBBoxCoder):
    def __init__(self,
                 image_size = (200,200),
                 target_means=(0., 0.),
                 target_stds=(0.5, 0.5)):
    ...
    def encode(self, gt_offsets):
        if isinstance(gt_offsets, list):
            gt_offsets = gt_offsets[0]
        bboxes = torch.Tensor([[0,0,self.image_size[0],self.image_size[1]]]).to(gt_offsets.device).repeat(gt_offsets.size(0),1)
        encoded_offsets = offset2delta(bboxes, gt_offsets, self.means, self.stds)
        return encoded_offsets
    ...
    def decode(self,
               ...
               max_shape=None,
               wh_ratio_clip=16 / 1000):
        if isinstance(pred_offsets, list):
            pred_offsets = pred_offsets[0]
        bboxes = torch.Tensor([[0,0,self.image_size[0],self.image_size[1]]]).to(pred_offsets.device).repeat(pred_offsets.size(0),1)
        decoded_offsets = delta2offset(bboxes, pred_offsets, self.means, self.stds,
                                    max_shape, wh_ratio_clip)

        return decoded_offsets
@BBOX_CODERS.register_module()
class DeltaXYOffsetCoder_Transformer2(BaseBBoxCoder):
    def __init__(self,
                 image_size = (200,200),
                 target_means=(0., 0.),
                 target_stds=(0.5, 0.5)):
    ...
    def encode(self, gt_offsets):
        if isinstance(gt_offsets, list):
            gt_offsets = gt_offsets[0]
        bboxes = torch.Tensor([[self.image_size[0],self.image_size[1]]]).to(gt_offsets.device).repeat(gt_offsets.size(0),1)
        # encoded_offsets = offset2delta(bboxes, gt_offsets, self.means, self.stds)
        encoded_offsets = gt_offsets/bboxes
        return encoded_offsets
    ...
    def decode(self,
        ...
        if isinstance(pred_offsets, list):
            pred_offsets = pred_offsets[0]
        bboxes = torch.Tensor([[0,0,self.image_size[0],self.image_size[1]]]).to(pred_offsets.device).repeat(pred_offsets.size(0),1)
        decoded_offsets = pred_offsets*bboxes
        return decoded_offsets
