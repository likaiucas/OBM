...
class OffsetDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # offset_head_hidden_dim: int = 256,
        ...
        offset_coder=dict(
            type='DeltaXYOffsetCoder_Transformer2',
        ...
        for i in range(num_fcs):
            in_channels = (
                64*64*256 if i == 0 else transformer_dim)
    ...
    #     x, src = self.predict_offset(image_embeddings, image_pe, sparse_prompt_embeddings, dense_prompt_embeddings,)
    ...
    def forward_train(self,
                ...
                **kwargs):
        # print(f'length of offset: {gt_offsets[0].shape[0]}')
        _, src = self.predict_offset(image_embeddings, image_pe, sparse_prompt_embeddings, dense_prompt_embeddings,)
        x = src.view(src.shape[0], -1)
        for fc in self.fcs:
            x = self.relu(fc(x))
        offset = self.fc_offset(x)
        offset_targets = self.offset_coder.encode(gt_offsets)   
    ...
    def predict_offset(self,
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens) # queries:(1,3,256) ,  keys:(1,4096,256)  <-  (图片特征， 位置信息，（output，prompt）)
        # offset_token_out = hs[:, 0, :] # [1,256]
        offset_token_out = hs.view(b,-1).contiguous()
        # src = src.transpose(1, 2).view(b, c, h, w) # [1,256,64,64] <- [1,4096,256]
        return offset_token_out, src
...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight], dim=0) #[5,256] <- [1,256], [4,256]
        output_tokens = output_tokens.unsqueeze(0).expand(sparse_prompt_embeddings.size(0), -1, -1)
