...
class OffsetHeadExpandFeature(nn.Module):
    ...
    def offset_fusion(self, offset_pred, model='max'):
        ...
        if model == 'mean':
            ...
            # offset_values /= 1
            offset_values /= self.expand_feature_num
        elif model == 'max':
            ...
            offset_values = torch.cat([torch.min(torch.abs(offset_value_x), dim=1)[0].unsqueeze(dim=1), torch.min(torch.abs(offset_value_y), dim=1)[0].unsqueeze(dim=1)], dim=1)
