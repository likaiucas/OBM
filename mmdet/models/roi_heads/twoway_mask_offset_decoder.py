    ...
    def __init__(
        ...
        *,
        ...
        for i in range(4):
            ...
            fc = nn.Linear(in_channels, hidden_dim, False)
            # nn.init.kaiming_normal_(fc.weight, mode='fan_out', nonlinearity='relu')
            self.fcs.append(fc)
        self.fc_offset = nn.Linear(hidden_dim, 2, False)
        ...
        if self.offset_aug_length:
            ...
            self.offset_aug_fc_offset = nn.ModuleList(nn.Linear(hidden_dim, 2, False) for i in range(self.offset_aug_length))
            # normal_init(self.offset_aug_fc_offset, std=0.01)
            for i in range(self.offset_aug_length):
                fcs = nn.ModuleList()
                for i in range(4):
                    in_channels = (
                        transformer_dim if i == 0 else hidden_dim)
                    fcs.append(nn.Linear(in_channels, hidden_dim, False))
