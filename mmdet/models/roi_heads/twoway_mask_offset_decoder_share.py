...
class MaskDecoder_seg_share(nn.Module):
    # __init__方法:
    # 1. 输入参数:
    ...
    def __init__(
        ...
        *,
        ...
        for i in range(4):
            in_channels = (
                transformer_dim if i == 0 else hidden_dim)
            fc = nn.Linear(in_channels, hidden_dim)
            # nn.init.kaiming_normal_(fc.weight, mode='fan_out', nonlinearity='relu')
            self.fcs.append(fc)
        self.fc_offset = nn.Linear(hidden_dim, 2)
        normal_init(self.fc_offset, std=0.01)
        # nn.init.kaiming_normal_(self.fc_offset.weight, mode='fan_out', nonlinearity='relu')

        
        self.offset_aug_length = len(offset_aug) if offset_aug else 0
        # self.offset_aug_query = nn.ModuleList(nn.Embedding(1, transformer_dim) for i in range(self.offset_aug_length)) if offset_aug else None
        self.offset_aug_coder = [build_bbox_coder(n) for n in offset_aug] if offset_aug else None
        
        if self.offset_aug_length:
            self.offset_aug_fcs = nn.ModuleList()
            offset_aug_fc_offset  = []
            for i in range(self.offset_aug_length):
                f = nn.Linear(hidden_dim, 2)
                normal_init(f, std=0.01)
                offset_aug_fc_offset.append(f)
            self.offset_aug_fc_offset = nn.ModuleList(offset_aug_fc_offset)
            ...
            for i in range(self.offset_aug_length):
                ...
                for i in range(4):
                    in_channels = (
                        transformer_dim if i == 0 else hidden_dim)
                    fcs.append(nn.Linear(in_channels, hidden_dim))
                self.offset_aug_fcs.append(fcs)             
    ...
    def forward_train(self,
        ...
        masks, prob, offset, offset_aug = self.predict_offset_masks(image_embeddings, image_pe, sparse_prompt_embeddings, dense_prompt_embeddings,)
        ...
        gt_building_masks = self.get_masks(gt_building_masks[0], masks, masks.shape[-1]/gt_building_masks[0].width)
        
        roof1, roof2, building1, building2 = masks[:,0,:,:], masks[:,1,:,:], masks[:,2,:,:],masks[:,3,:,:]
        loss_mask_roof1 = self.loss_mask_roof(roof1, gt_masks.float())
        loss_mask_roof2 = self.loss_mask_roof(roof2, gt_masks.float())
        loss_mask_building1 = self.loss_mask_roof(building1, gt_building_masks.float())
        loss_mask_building2 = self.loss_mask_roof(building2, gt_building_masks.float())
        # loss_iou1 = self.loss_iou(prob1,self.iou(gt_masks, roof1).float())
        # loss_iou2 = self.loss_iou(prob2,self.iou(gt_masks, roof2).float())
        # loss_iou3 = self.loss_iou(prob3,self.iou(gt_building_masks, building1).float())
        # loss_iou4 = self.loss_iou(prob4,self.iou(gt_building_masks, building2).float())
        losses = dict(loss_offset=loss_offset, loss_mask1=loss_mask_roof1, loss_mask2=loss_mask_roof2, 
                    loss_mask3=loss_mask_building1,loss_mask4=loss_mask_building2,)  
        if self.offset_aug_length:
            for i in range(self.offset_aug_length):
                offset_aug_targets = self.offset_aug_coder[i].encode(gt_offsets)   
                losses[f'loss_offset_aug{i}'] = self.loss_offset(offset_aug[i], offset_aug_targets)
        return losses
    ...
    def predict_offset_masks(
        ...
        if self.offset_aug_length:
            ...
            for i in range(self.offset_aug_length):
                # x = offset_tokens[i+1]
                x = offset_tokens[0]
                for fc in self.offset_aug_fcs[i]:
                    x = self.relu(fc(x))
                offset_aug.append(self.offset_aug_fc_offset[i](x))
            return masks, iou_pred, offset, offset_aug
        # Prepare output
        return masks, iou_pred, offset, None
    ...
    def predict_masks(
        ...
        output_tokens = [self.iou_token.weight, self.mask_tokens.weight, self.offset_query.weight]
        # if self.offset_aug_length:
        #     for i in range(self.offset_aug_length):
        #         output_tokens.append(self.offset_aug_query[i].weight)
        ...
        if self.offset_aug_length:
            for i in range(self.offset_aug_length):
                offset_tokens_out.append(offset_tokens_out)
