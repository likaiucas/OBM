...
def noise(x, mean=0):
    # offset = torch.tensor(torch.rand(x.shape).cuda()*mean, dtype=torch.int)
...
# prompt boxes        
    def forward_test(self, 
        ...
        if self.noise_box is not None:
            gt_bboxes =[ noise(gt_bboxes[0][0], self.noise_box).unsqueeze(0)]
        ...
        masks = self.postprocess_masks(masks, (1024,1024), (1024,1024))
        ...
        return return_box[0][0], masks, offset
